...
public class DifferenceBetweenDate {
    public static Duration durationBetweenTime(LocalTime localTime1, LocalTime localTime2) {
        if (localTime1 == null || localTime2 == null)
            return null;
        Duration dbt = Duration.between(localTime1, localTime2);
    ...
    public static Period periodBetweenDate(LocalDate date1, LocalDate date2) {
        ...
        Period pbd = Period.between(date1, date2);
        if (pbd.isNegative()){
            pbd = pbd.negated();
        }
        return pbd;
    ...
    //     System.out.println(DifferenceBetweenDate.numberOfHoursBetweenDateTime(LocalDateTime.of(2022, 4, 12, 16, 18, 56),
    //             LocalDateTime.of(2022, 5, 10, 21, 54, 56)));
    // }
}
